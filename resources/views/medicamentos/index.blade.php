...
<body>
  ...
  @if(session('deleted'))
    <div class="toast" id="toast">
      <div class="toast-icon danger">
        <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
          <path d="M2.5 7.5l5-5M7.5 7.5l-5-5" stroke="#fff" stroke-width="1.5" stroke-linecap="round" />
        </svg>
      </div>
      {{ session('deleted') }}
    </div>
  ...
  <form action="{{ route('medicamentos.index') }}" method="GET" class="toolbar">
    ...
    <select name="estado" onchange="this.form.submit()">
      ...
      <option value="Próximo a caducar" {{ request('estado') == 'Próximo a caducar' ? 'selected' : '' }}>Próximo a caducar
      </option>
  ...
  <div class="table-card">
          ...
          <tr>
            ...
            <td>
              <div class="row-actions">
                <button type="button" class="btn-icon edit" title="Editar" data-id="{{ $m->id }}"
                  data-nombre="{{ $m->nombre }}" data-cantidad="{{ $m->cantidad }}"
                  data-fecha="{{ $m->fecha_caducidad->format('Y-m-d') }}" data-categoria="{{ $m->id_categoria }}"
                  data-accion="{{ $m->accion_recomendada }}" onclick="abrirEditar(this)">
                  <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
                    <path d="M11.5 2.5a2.121 2.121 0 013 3L5 15H2v-3L11.5 2.5z" stroke="currentColor" stroke-width="1.4"
                      stroke-linecap="round" stroke-linejoin="round" />
                  </svg>
                </button>
                <button type="button" class="btn-icon delete" title="Eliminar" data-id="{{ $m->id }}"
                  data-nombre="{{ $m->nombre }}" onclick="abrirEliminar(this)">
                  <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
                    <path d="M2 4h12M5 4V2h6v2M6 7v5M10 7v5M3 4l1 9h8l1-9" stroke="currentColor" stroke-width="1.4"
                      stroke-linecap="round" stroke-linejoin="round" />
                  </svg>
                </button>
              </div>
    ...
    <div class="table-footer">Mostrando {{ $medicamentos->count() }} de {{ $stats['total'] }} registros encontrados
    </div>
  </div>
          ...
          <div class="form-grid">
            <div class="form-group full">
              ...
              <input type="text" id="c_nombre" name="nombre"
                ...
                placeholder="Ej. Paracetamol 500mg" value="{{ !session('editing') ? old('nombre') : '' }}"
                autocomplete="off">
            ...
            <div class="form-group">
              ...
              <select id="c_categoria" name="id_categoria"
                ...
                @foreach($categorias as $cat)
                  <option value="{{ $cat->id_categoria }}" {{ (!session('editing') && old('id_categoria') == $cat->id_categoria) ? 'selected' : '' }}>
                    {{ $cat->nombre_categoria }}
                  </option>
                @endforeach
              </select>
              @if(!session('editing')) @error('id_categoria') <span class="error-msg">{{ $message }}</span> @enderror
              @endif
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              @if(!session('editing')) @error('fecha_caducidad') <span class="error-msg">{{ $message }}</span> @enderror
              @endif
            </div>
            <div class="form-group full">
      ...
      <div class="delete-body">
        <div class="delete-icon">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
            <path d="M3 6h18M8 6V4h8v2M19 6l-1 14H6L5 6M10 11v6M14 11v6" stroke="#ef4444" stroke-width="1.8"
              stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </div>
